...
The table/prose remains a reviewable proposal. Big Pickle validated the analyzer (149 full-suite tests, no skips) and reproduced the expected counts; the [measurement report](../experiments/codons/family-variant-analysis-report.json) records candidate and shuffled controls. An [authored 17-codon forager](authored-atomic-forager.md) now explores expressiveness, with its static review validated by Big Pickle (153 full-suite tests). Before freezing a language version: validate the enumerator against hand-worked cases; review measured controls and the family heuristic; assess whether this machine can express the authored forager economically; and decide whether the limited branches, two-synonym allocation and memory/register split are appropriate. Then implement the new language alongside source and forager-v1, retaining compiled default, and compare execution traces before mutation or sexual reproduction is enabled.
...
## Experimental decoder and state foundation

`worldish.atomic_state` now provides strict decoding and suspended-state validation for explicitly supplied draft candidates. It is not registered as a simulation language and does not execute instructions. The candidate remains provisional; its content hash must be supplied explicitly when decoding, so a changed candidate requires a deliberate new pin.

The decoder retains exact raw bases and immutable codon/operation tuples. Empty genomes are accepted; odd lengths, invalid bases, formatting and more than 1024 codons are rejected. Every operation in the supplied table must belong to the proposed vocabulary, even if the genome does not use it. Raw identity is SHA-256 of ASCII bases. Decoded-program identity hashes the candidate hash together with the operation sequence: synonyms share that identity within one candidate while retaining distinct raw identities. These diagnostic hashes are not production language versions or runtime cache keys.

Fresh state contains `a`, `r`, eight `memory` slots, slot pointer `p`, heading `h`, `ip`, pending `prayer` and prepaid `credit`. All numeric values start at zero, prayer starts at `Live`, and each agent owns its slot list. `validate_state` rejects extra/missing fields, booleans masquerading as integers, invalid signed-16 values, slot/heading indices outside 0–7, invalid pending requests and credit outside 0 through batch-size minus one. It requires the originating program length and compute batch size, and returns a defensive copy. Suspended instruction pointers address an existing codon; empty programs require zero. Completion must normalize the pointer before suspension. This is a machine-data boundary, not a world checkpoint or permission to change policies while resuming.

Tests and Big Pickle's validation instructions are in the [decoder/state handoff](../.memory/wiki/notes/atomic-state-validation.md). Opcode execution, arithmetic saturation, instruction charging and method parity remain tasks [D2–D6](evolution-tasklist.md#d-authored-codon-forager).
